...
class IAMDataset(dataset.ArrayDataset):
    ...
    def __init__(self, parse_method,
                 #root=os.path.join(os.path.dirname('/content/'), 'dataset', 'iamdataset'),
                 root='/content/dataset/iamdataset/',
        ...
        print(self.image_data_file_name)
        self._root = root
    ...
    def _save_dataframe_chunks(self, df, name):
        for i, df_split in enumerate(np.array_split(df, 4)):
            filename = name[:-5] + str(i) + ".plk" # remove *.pkl in the filename
